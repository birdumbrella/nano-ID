...
basic.objects = c(ls(),"build.raw.signal.five.mers.add.on.list","col.names","z.score.rescaling","five.mer.RNA.pore.model","non.T.containing")
...
for (j in 1:length(index.list)){
  ...
  raw.signal.five.mers.add.on.subset = foreach(n = names(bam)[index.list[[j]]],.noexport = setdiff(ls(),c("bam","read.to.fast5.name","read.sequence.list","sequencing.summary"))) %dopar% build.raw.signal.five.mers.add.on.list(n)
  raw.signal.five.mers.add.on.list[[j]] = Reduce('rbind',raw.signal.five.mers.add.on.subset)
}

raw.signal.five.mers.add.on = Reduce('rbind',raw.signal.five.mers.add.on.list)
colnames(raw.signal.five.mers.add.on) = col.names
rownames(raw.signal.five.mers.add.on) = raw.signal.five.mers.add.on[,"read.name"]
raw.signal.five.mers.add.on = apply(raw.signal.five.mers.add.on[,setdiff(col.names,"read.name")],c(1,2),as.numeric)
raw.signal.five.mers.add.on = t(apply(raw.signal.five.mers.add.on,1,function(x){z.score.rescaling(five.mer.RNA.pore.model[names(x[!is.na(x) & non.T.containing]),"mean"],x[!is.na(x) & non.T.containing],x)}))

save(raw.signal.five.mers.add.on,file=file.path("...","raw.signal.five.mers.add.on.RData"))
